...
if ($id <= 0 || empty($name) || empty($email)) {
    echo json_encode(['status' => 'error', 'message' => 'Valid Employee ID, Name, and Email are required.']);
    exit;
}

$update_obj = new employee_ADD_UPDATE();
$update_obj->set_id($id);
$update_obj->set_data($name, $email, $dept, $role, $status, $joined);
...
if ($res) {
    echo json_encode([
        ...
        'data'    => [
            'id'       => $id,
            'initials' => $update_obj->get_initials(),
            'name'     => $name,
            'email'    => $email,
            'dept'     => $dept,
            'role'     => $role,
